...
use crate::utils::{address_to_bytes, bytes_to_address, u32_to_bytes};

#[cfg(not(feature = "sparse_addr"))]
use crate::params::{SPX_FULL_HEIGHT, SPX_D};
...
// Dense address functions (default, used for sha2/shake)
#[cfg(not(feature = "sparse_addr"))]
mod dense {
    use super::*;
...
// Sparse address functions (for Cairo compatibility with blake2s)
...
mod sparse {
    ...
    /// Specify which Merkle tree within the level (the "tree address") we're working on
    pub fn set_tree_addr(addr: &mut [u32], tree: u64) {
        addr[SPX_OFFSET_TREE_HI] = (tree >> 32) as u32;
        addr[SPX_OFFSET_TREE_LO] = tree as u32;
    }

    /// Specify the reason we'll use this address structure for, that is, what
    /// hash will we compute with it.
    pub fn set_type(addr: &mut [u32], addr_type: u32) {
        addr[SPX_OFFSET_TYPE] = addr_type;
    }

    /// Copy the layer and tree fields of the address structure.
    pub fn copy_subtree_addr(out: &mut [u32], input: &mut [u32]) {
        out[SPX_OFFSET_LAYER] = input[SPX_OFFSET_LAYER];
        out[SPX_OFFSET_TREE_HI] = input[SPX_OFFSET_TREE_HI];
        out[SPX_OFFSET_TREE_LO] = input[SPX_OFFSET_TREE_LO];
    }

    /// Specify which Merkle leaf we're working on; that is, which OTS keypair
    /// we're talking about.
    pub fn set_keypair_addr(addr: &mut [u32], keypair: u32) {
        addr[SPX_OFFSET_KP_ADDR] = keypair;
    }
    ...
    pub fn copy_keypair_addr(out: &mut [u32], input: &[u32]) {
        ...
        out[SPX_OFFSET_TREE_HI] = input[SPX_OFFSET_TREE_HI];
        out[SPX_OFFSET_TREE_LO] = input[SPX_OFFSET_TREE_LO];
        out[SPX_OFFSET_KP_ADDR] = input[SPX_OFFSET_KP_ADDR];
    }

    /// Specify which Merkle chain within the OTS we're working with (chain address)
    /// In sparse format, chain is stored in upper bits of wots_addr (chain * 0x100)
    /// to match Cairo's encoding: wots_addr = chain_idx * 0x100 + hash_position
    pub fn set_chain_addr(addr: &mut [u32], chain: u32) {
        // Store chain in upper bits, clear lower bits (hash will be set by set_hash_addr)
        addr[SPX_OFFSET_WOTS_ADDR] = chain << 8;
    }

    /// Specify where in the Merkle chain we are (hash address)
    /// In sparse format, hash is stored in lower 8 bits of wots_addr
    /// to match Cairo's encoding: wots_addr = chain_idx * 0x100 + hash_position
    pub fn set_hash_addr(addr: &mut [u32], hash: u32) {
        // Store hash in lower 8 bits, preserve upper bits for chain
        let chain = addr[SPX_OFFSET_WOTS_ADDR] >> 8;
        addr[SPX_OFFSET_WOTS_ADDR] = (chain << 8) | (hash & 0xFF);
    }

    /// Specify the height of the node in the Merkle/FORS tree
    pub fn set_tree_height(addr: &mut [u32], tree_height: u32) {
    ...
    /// Specify the distance from the left edge of the node in the Merkle/FORS tree
    pub fn set_tree_index(addr: &mut [u32], tree_index: u32) {
        addr[SPX_OFFSET_TREE_INDEX] = tree_index;
    }
}
